...
def guardar_resultados_json(resultados, paciente_id):
    ...
    datos = {
        ...
        "modulo": "Ley de Fitts",
        "metrica_principal": {
            "nombre": "Throughput (TP)",
            "descripcion": "Cantidad de informacion procesada por unidad de tiempo (bits/s). Mide la eficiencia combinada de velocidad y precision del movimiento apuntado.",
            "unidad": "bits/s",
            "valor": round(resultados.get("tp", 0), 4)
        },
        "metricas_secundarias": {
            "tiempo_movimiento_promedio_s": round(resultados.get("mt", 0), 4),
            "error_promedio_px": round(resultados.get("error", 0), 4),
            "pendiente_b": round(resultados.get("b", 0), 4)
        },
        "resumen": {
            "puntaje": resultados.get("puntaje", 0),
            "intentos": resultados.get("intentos", 0),
